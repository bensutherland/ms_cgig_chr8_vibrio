...
# Requires that ms_cgig_chr8 is in the same directory as simple_pop_stats (same level)
# Clear workspace, launch simple_pop_stats

# Load simulate_cross function
source("../ms_cgig_chr8/01_scripts/simulate_cross.R")
...
geno.df <- obj$tab
geno.df[1:5,1:15] # these are the sample names that will be used

# keep only the alternate allele
geno.df <- geno.df[, grep(pattern = "\\.1$", x = colnames(x = geno.df))] # Keep only the alternate allele value
str(geno.df) # ensure integer
is.numeric(geno.df[,1])
# geno.df <- t(geno.df)
# str(geno.df)
# is.numeric(geno.df[,1])

geno.df[1:3,1:15] # only contains the count of alternate alleles
...
# isolate just cross info
cross.df <- cross_and_pheno.df[, c("family", "dam", "sire")]
head(cross.df)

geno.df[1:5,1:5]

# Create an empty matrix with the number of rows equal to the number of families, and the number cols equal to number loci
geno_sim <- matrix(data = NA, nrow = nrow(cross.df), ncol = ncol(geno.df))
dim(geno_sim)
str(geno_sim)
...
for(i in 1:nrow(cross.df)){
  
  geno_sim[i,] = apply(geno.df[match(c(cross.df$dam[i], cross.df$sire[i]), rownames(geno.df)), ], 2, simulate_cross)
